...
<html>
    <head>
        <title>Kontributor | Edit Artikel</title>
        <meta charset="utf-8">
    ...
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        ...
        <?php echo form_open_multipart('kontributor/edit/'.$artikel[0]['id']);?>
            <div class="form-group">
                <h4 for="text1" class="control-label">Judul Artikel</h4>
                <input type="text" id="judul" placeholder="Judul" class="form-control" name="judul" value="<?php echo $artikel[0]['judul'] ?>" />
            </div>
            <h4>Isi Artikel</h4>
            <?php echo $this->ckeditor->editor('description', $artikel[0]['isi']);?><?php echo form_error('description','<p class="error">'); ?>
            <br><input type="submit" name="submit" value="Save & Publish" id="save" class="save btn btn-primary" />
            <a class="btn btn-default" href="<?php echo base_url('kontributor/data_blog') ?>">Back to Data Artikel</a>
            <a class="btn btn-info" href="<?php echo base_url('kontributor') ?>">Back to Contributor Page</a>
            ...
            <div class="form-group">
                <select data-placeholder="Tags Artikel" id="tag-artikel" name="tags[]" multiple class="form-control chzn-select" tabindex="8">
                    ...
                    <?php foreach ($tags->result_array() as $tag) { ?>
                        <?php if (in_array($tag['tags'], $tags_artikel)) { ?>
                            <option value="<?php echo $tag['id'] ?>" selected="selected"><?php echo $tag['tags'] ?></option>
                        <?php } else { ?>
            ...
            <h4>Gambar Artikel</h4>
                <div class="panel">
                    <div class="panel-body">
                        ...
                        <?php } else { ?>
                            <img width="250px" height="103,75px" src="<?php echo base_url() ?>images/artikel/gambar/<?php echo $artikel[0]['gambar'] ?>">
                        <?php } ?>
                    </div>
                </div>
            <h4>Upload Gambar</h4>
